...
- **AI-assisted compliance is built, not just promised.** Live at **`/terminal/compliance`** (and agent-callable at **`/api/compliance`**): a deposit-boundary screen that runs a deterministic rule engine — sanctions/AML screening (sample OFAC SDN set, pluggable for Chainalysis/TRM/a live OFAC feed), USDY KYC / transfer-restriction enforcement, the **live on-chain AI risk-state** (our forecast/anomaly engine via RiskManager, which Freezes deposits on low-confidence/stale conditions), and transaction-monitoring (enhanced due diligence over a notional threshold) — then an LLM writes the plain-English ALLOW/REVIEW/DENY recommendation. The AI is grounded in the concrete checks; it explains the decision, it doesn't invent it.

This is a hackathon-scope posture (testnet; sample sanctions list + stubbed KYC allow-list, swappable behind the same boundaries; advisory/no-custody) stated honestly — but the architecture is compliance-*automating*, not just compliance-aware.
...
The Grand Champion nomination is justified by full-stack depth (18 production contracts + a 7-agent AI forecasting swarm + indexer + frontend) and genuine Mantle composition.

## What was built (concrete)

- **Smart contracts (Foundry, Solidity 0.8.24, 224/224 tests):** AgentRegistry (ERC-8004 soulbound + per-category top-20), PredictionMarket (commit-reveal escrow), ResolutionEngine, ScoringEngine + RangeCrpsScorer (closed-form CRPS with a Python reference matching the Solidity bit-for-bit), MethAprResolver + AaveMantleTvlResolver + UsdyApyResolver + **PythSpotResolver** (live Pyth MNT/USD — a keeper pins the Hermes-verified price at the resolution block, first-write-wins, so the outcome is caller- and timing-independent), CompositeFeed (rank-weighted, calibration-multiplier), BonusDistributor (pull-claim epochs), SubscriptionGate, DemoFeedConsumer (business-logic views), and the AI × RWA consumers **YieldAllocator** (confidence-weighted dynamic allocation across mETH + USDY) + **RiskManager** (automated risk state from forecast confidence + freshness), + mocks. Full deploy + smoke + end-to-end pipeline scripts/tests.
...
| Deployed addresses | 20 contracts (18 production + 2 mocks) — full table in [`README.md`](../README.md#-deployed-contracts); source: `contracts/deployments/mantle-sepolia.json`. Headline: CompositeFeed `0x695aC142…`, YieldAllocator `0xD68ABfAD…`, RiskManager `0xbe7e481D…`, DemoFeedConsumer `0xAe56da72…`, PythSpotResolver `0x5CEa3D92…` |
