...
imply `MNT_USD_SPOT`'s feed line is populated (it has 1 keeper-resolved prediction, #836 — the mechanism is proven, but ≥10 are needed before the composite feed shows a value).
...
- **Set the leaderboard category tab to `AAVE_MANTLE_TVL_24H`** — the category where the ranking cleanly reads **naive < ARIMA < reasoner** (reasoner #1, ARIMA #2, naive #3), so "click the top-ranked agent" lands on the DeepSeek reasoner (`agent/2`) with the real pinned trace. (All three of METH/AAVE/USDY have the naive control resolved; AAVE has the cleanest ordering. `MNT_USD_SPOT` now has its first keeper-resolved prediction — #836, scored 99.8% against a keeper-pinned Pyth price — but with 1 resolved the feed line is still empty, so cite it as the **live-Pyth mechanism proof** (`getPrediction(836)` + `snapshotPrice`), don't lead the leaderboard with it.)
...
**Never say "live" when the pill says "snapshot."** `MNT_USD_SPOT` **is** live on testnet with its first keeper-resolved prediction (#836, 99.8%) — but with 1 resolved its composite *feed line* is still empty, so never imply the feed already shows a populated MNT value. A single falsifiable claim on stage costs more than any dropped feature.
...
- **"Is the Pyth live-price category deployed?"** — "Straight answer: **yes, and the keeper flow is live on-chain.** PythSpotResolver `0x5CEa…5fF4` is registered on ResolutionEngine, PredictionMarket, and CompositeFeed, and prediction **836** resolved **99.8%** against a keeper-pinned Pyth price of **$0.4221** — `getPrediction(836)` and `snapshotPrice(40694247)` are both verifiable on Mantlescan right now, and the keeper `record()` tx is on-chain. Over 30 tests including the drift-exploit test. The one honest caveat: only 1 resolved so far, so the composite *feed line* (which needs ≥10 resolved per the top-agent gate) isn't populated yet — the mechanism is proven, the track record is accruing."
...
- **MNT_USD_SPOT Pyth spot category is live** on Mantle Sepolia (PythSpotResolver `0x5CEa…5fF4`; registered on ResolutionEngine, PredictionMarket, CompositeFeed) — Hermes-verifiable truth. The keeper flow is proven on-chain: prediction **836** resolved 99.8% against a keeper-pinned Pyth snapshot ($0.4221); `/api/feed` now serves MNT. Caveat: only 1 resolved so far, so the composite feed line (needs ≥10 per the top-agent gate) isn't populated yet.
...
- `MNT_USD_SPOT` has 1 keeper-resolved prediction (#836, 99.8%) — the keeper→record→resolve→CRPS flow is proven on-chain, but the field is thin, so don't imply a populated feed or full 3-agent parity on it.

**ROADMAP (state as "next," never as done):**
- MNT_USD_SPOT Pyth category is **live** on Mantle Sepolia (resolver `0x5CEa…5fF4`); the keeper flow is proven on-chain (prediction 836 resolved 99.8% against a keeper-pinned Pyth snapshot; `/api/feed` now serves MNT). What's "next" is enough resolved rounds (≥10) to populate the composite feed line.
...
**North star:** un-fakeable because *committed before the outcome, graded on-chain against verifiable truth.* The money moment is Beat 2. Cut the insights beat first if long. Never claim a surface is live when its pill says snapshot; MNT_USD_SPOT's keeper flow is proven (prediction 836, 99.8%), but never imply its composite feed line is populated yet (1 resolved, ≥10 needed).
