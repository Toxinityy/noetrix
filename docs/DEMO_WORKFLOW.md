...
| **0:48–1:38** · 🎯 **MONEY MOMENT** | Click the top agent → `/terminal/agent/2`. Read the "Illustrative profile" banner aloud, show the reasoning trace → switch to the **IPFS** tab → switch to **Mantlescan `getPrediction(140)`** | *"Here's the proof. This is the reasoner's actual forecast — full reasoning, hash-committed to IPFS and anchored on-chain **before the outcome existed**. A 200-block cutoff means it physically cannot see the answer and back-fit. [IPFS] Same content, content-addressed — I can't edit it. [Mantlescan] On-chain: agent 2, commit block earlier than the resolution block. No hindsight, no cherry-pick — a track record you cannot fake, and you can verify it yourselves right now."* |
...
**Verify-it-yourself handles (say the judges can check these unaided):** IPFS `QmREFScRDmHTm82P391LrmSds1BSutPVHDRCHKgJhm3Wvy` · Mantlescan PredictionMarket `0xaa92b0434F89a17F2275b655c6fA459C43813f22` → `getPrediction(140)` → agentId 2, commit block **39,649,491 < 39,649,841** resolution · Pyth truth on **hermes.pyth.network**.
...
| 1:05–2:05 | 🎯 **MONEY MOMENT** | Click the top-ranked reasoner → `/terminal/agent/2`. **Read the "Illustrative profile" banner aloud**, scroll to the featured trace. Switch to IPFS tab (`QmREF…3Wvy`), then Mantlescan (`getPrediction(140)`) | Full reasoning → forecast band + confidence; the same content pinned on IPFS; the on-chain commit whose block precedes the resolution block | *"This is the moment. A real forecast, full reasoning hash-committed to IPFS and anchored on-chain **before the outcome existed**. The 200-block submission cutoff means the agent physically cannot see the answer and back-fit. And notice — we tell you ourselves which parts are demo-shaped. An honest benchmark says what it can't yet prove."* | **Verifiability & auditability (B-15)**, Technical (A-15), Innovation (A-10) |
...
4. Switch to the IPFS tab (`QmREFScRDmHTm82P391LrmSds1BSutPVHDRCHKgJhm3Wvy`): *"This exact prompt, reasoning, and forecast is content-hashed and pinned. The hash on-chain matches this file."*
5. Switch to Mantlescan (PredictionMarket → `getPrediction(140)`): *"And here's the on-chain record — agent 2, the content hash matches the IPFS file, and the commit block is earlier than the resolution block. The forecast provably predated the outcome."*
...
- **Explorer:** on `sepolia.mantlescan.xyz`, call PredictionMarket `getPrediction(140)` and confirm agentId 2, contentHash == the CID, and commit block < resolution block. Recompute the CRPS grade from the committed band and the resolved truth — the scoring is deterministic and open.
...
- Agent #2 (DeepSeek) prediction #140 with IPFS-pinned reasoning `QmREF…3Wvy` — real, independently verifiable (confirm via `getPrediction(140)` + a public or the Pinata gateway).
