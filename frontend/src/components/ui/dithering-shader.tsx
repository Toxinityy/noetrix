...
interface DitheringShaderProps {
  /** Width in CSS pixels. If `fill` is true, this is ignored. */
  width?: number;
  /** Height in CSS pixels. If `fill` is true, this is ignored. */
  height?: number;
  /** When true, the canvas fills its parent and resizes with it via ResizeObserver. */
  fill?: boolean;
  colorBack?: string;
  colorFront?: string;
  ...
  type?: DitheringType;
  pxSize?: number;
  /** Animation speed multiplier. Pass 0 to freeze (used for prefers-reduced-motion). */
  speed?: number;
  className?: string;
...
function createShader(gl: WebGL2RenderingContext, type: number, source: string): WebGLShader | null {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error("DitheringShader compile error: " + gl.getShaderInfoLog(shader));
...
function createProgram(
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error("DitheringShader link error: " + gl.getProgramInfoLog(program));
...
export function DitheringShader({
  ...
  return (
    <div ref={wrapRef} className={className} style={containerStyle}>
      <canvas
        ref={canvasRef}
        style={{
          display: "block",
