...
- **Indexer crash-loops on public-RPC 429s** (HTTP 000). Agents/resolver/refresher tolerate the public RPC; the indexer's parallel eth_getLogs gets rate-limited to death. FIX: a free Alchemy/Infura Mantle Sepolia URL → set `PONDER_RPC_URL_MANTLE_SEPOLIA` in `indexer/.env.local` + bot `MANTLE_SEPOLIA_RPC` → restart. Until then the leaderboard page has no live data (data is on-chain; `/rwa` reads contracts directly).
