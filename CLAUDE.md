...
### 2026-07-02 (latest) — Believability audit (4-lens Workflow) + Batch A credibility fixes
**Type:** Audit (multi-agent Workflow, ultracode) + 15-file remediation. User asked "what's still not believable?" + "is the CEO-review plan all checked?" Ran a grounded 4-lens Workflow (ceo-checklist / on-chain-skeptic / frontend-overclaims / docs-claims — 786k subagent tokens, everything verified against the live chain/explorer/site, NOT the masterdoc), then executed "Batch A" (the cheap, high-credibility fixes) on user approval. Branch `live-pyth-spot`.
- **CEO-review status surfaced:** 2026-06-04 (business framing) ~80% done (reputation-passport ✓, scorecard-first ✓, pricing-as-target ✓); gaps: "trustless" still in landing copy, DEMO_SCRIPT.md still "protocols pay", no external buyer reaction. 2026-07-01 (Pyth) DEPLOYED-not-MATURED: contract layer + wiring ✓, but zero resolved MNT_USD_SPOT track record, PythSpotResolver unverified on explorer, keeper path never run on-chain.
- **Batch A fixes (all shipped, verified tsc 0 · vitest 88 · lint 0 · next build 29 routes):**
  - **#1 self-falsifying money moment (HIGH):** `AgentDetailClient.tsx` REAL_TRACE_PREDICTION claimed "on-chain prediction #140", but on the LIVE PredictionMarket `getPrediction(140)` = agent-5/AAVE (the QmREF trace is real IPFS content but from an *earlier contract* deploy — nextPredictionId is now 836). Fix: reframed the trace as a real IPFS-pinned DeepSeek provenance payload (content-addressed, verifiable) and **decoupled the on-chain proof to agent 2's real current-contract track record** — `getPrediction(773)` (verified on-chain: agent 2, METH band 240–270, commit 40,106,259 < res 40,149,456, Resolved, score 956,667; one of 43 resolved). Updated the banner + panel pill + all `docs/DEMO_WORKFLOW.md` money-moment beats (was `getPrediction(140)` everywhere).
  - **#2 dead-address `/terminal/agents` page (HIGH):** hardcoded CONTRACTS were the abandoned 2026-05-30 deploy → replaced with live AgentRegistry/PredictionMarket/ResolutionEngine/CompositeFeed.
  - **#3/#8 docs (HIGH/MED):** README+SUBMISSION said "19 contracts / all verified / 191 tests" — actual is **20 deployed (18 prod + 2 mock), 224 tests**, and PythSpotResolver is UNVERIFIED. Fixed counts, badges, added PythSpotResolver table row, replaced stale DemoFeedConsumer (`0xAe56da72…`) + RiskManager (`0xbe7e481D…`) addresses, softened "all verified" → "core verified, Pyth pending".
  - **#4 MNT_USD_SPOT dormant (HIGH):** fixed the dashboard fallthrough that rendered Aave's $142M as the "MNT/USD spot price" (excluded MNT from `ACTIVE_CATEGORY_IDS`), softened the "live Pyth price" description to roadmap framing, added MNT to `/api/feed` VALID.
  - **#5 fabricated Epoch/bonus-pool (MED):** removed the mock `RECENT_EPOCHS` "Epoch 1240" pill + "1.8 MNT bonus pool" stats from the genuinely-live leaderboard.
  - **#6 compliance "AI memo" (MED):** `/api/compliance` now tags `source: aiConfigured ? "ai-assisted" : "rule-based"` (OPENROUTER key unset on Vercel → it's rule-based); relabeled UI "AI compliance memo" → "Compliance memo", softened "an AI writes it".
  - **Copy honesty:** "trustless" → "verifiable" (HowItWorks); insights footer "live on-chain data" → "captured in a committed snapshot"; hero "14/20" → "7/20"; CategoriesShowcase 4/2/4 → 7 agents; FAQ/Footer pricing reconciled to "target production pricing" + testnet-MNT pilot; insights gate CTA no longer promises a self-serve checkout the UI lacks.
- **Batch B (operational — DONE this session):** (1) **verified all 20 contracts** on Mantlescan — `forge verify-contract` PythSpotResolver (the flagged one) + `verify.sh` for the rest, 0 failures; README now truthfully "All 20 source-verified". (2) **Executed the MNT_USD_SPOT keeper flow end-to-end on-chain** via a one-off (`agents/arima-baseline/scripts/oneoff-mnt.ts`): agent 1 committed+revealed a live-priced band → keeper (deployer) fetched a fresh Hermes update + `record()` pinned the real Pyth price **$0.4221** at resolutionBlock 40,694,247 → `resolve()` → **prediction 836 Resolved, CRPS score 998334 (99.8%)** → feed refreshed. This is the first on-chain `record()` call — directly refutes the audit's "keeper never called / dormant scaffolding". On-chain: `getPrediction(836)` status Resolved, `snapshotPrice(40694247)=42211500`, `recorded=true`, `getReputation(1,MNT).resolvedCount=1`. mockData MNT description + 7 DEMO_WORKFLOW honesty lines updated to reflect the keeper flow is proven (honest caveat kept: 1 resolved, feed line needs ≥10 per the top-agent gate). (3) **Regenerated the insights snapshot** (was 21 days stale @ block 39,805,025 → now @ 40,694,282, all 4 categories incl MNT). Still deferred: set OPENROUTER_API_KEY in Vercel for real AI compliance; durable server run to accrue MNT ≥10 (agent NORMAL_CADENCE_MS tightened 6h→1h this session to help).
- Also earlier this session: fixed wrong-agent leaderboard links, windowed the feed chart past the stopped-bot gap, added landing scroll-snap (snap-on-release, no delay, snappier ease). All deployed to https://noetrix.vercel.app.

### 2026-07-02 (later) — Fix wrong-agent links + window feed chart past stopped-bot gap; backfill/backtest analysis
**Type:** Build (frontend, 2 targeted fixes) + on-chain investigation. User reported (a) clicking a leaderboard agent opened the WRONG agent's detail page, and (b) asked whether the composite-feed "data holes" can be backfilled/backtested. Branch `live-pyth-spot`.
- **Wrong-agent link — root cause + fix (`frontend/src/lib/mockData.ts`):** the detail page resolves an agent via `getAgentById` → the mock `AGENTS` array, whose **ids 3–7 were stale fictional agents** (quant-grad-momentum, deepseek-reasoner-β/γ, ensemble-mean, naive-persistence) that mismatch the real on-chain `KNOWN_AGENTS` (Naive Baseline, Mean-Reversion, Momentum, EWMA-Volatility, Sentiment). So `getAgentById(6)` returned "deepseek-reasoner-γ" while the leaderboard linked #6 = EWMA-Volatility. **Fix:** `getAgentById` now reconciles **identity (name + kind)** to `KNOWN_AGENTS`/`inferKind` — the SAME registry the leaderboard links from — so every link lands on that agent. Reputation/equity/reasoning stay demo-shaped (already disclosed by the page's Illustrative-profile banner); only identity is made authoritative.
- **Reasoning-trace honesty (`AgentDetailClient.tsx`):** with ids 3–7 now correctly QUANT, the featured "reasoning trace" is gated to the real reasoner (id 2, its REAL pinned trace #140) or CLAUDE-kind agents only — a formula strategy (Momentum/EWMA) no longer shows a fabricated LLM trace. Banner text keys off `hasReasoning` so it doesn't reference a trace that isn't rendered.
- **Feed-chart windowing (`FeedClient.tsx`):** chart now slices to the **most-recent contiguous run** — detects the last inter-point gap > `DAY_BLOCKS` (a stopped-refresher outage vs the ~150-block cadence) and shows points from there. Header caption flips to "N updates · since feed resumed" when trimmed. Every point is still real on-chain data; it just stops a ~12-day dead zone from flattening the view. No gap → nothing trimmed.
- **Backfill/backtest investigation (chain-grounded):** paginated `cast logs` scan of `CompositeFeedRefreshed(METH)` over the chart window → **851 real events, but shaped as two dense live-run clusters (≈5-day + recent) around a ~12-day ZERO-event dead zone** (blocks ~40.12m→40.65m) where the refresher was simply stopped. Conclusion given to user: **backfill recovers nothing in the gap** (events never existed — not an indexer-capture miss); **backtesting the live feed is off-limits** (fabricating pre-outcome forecasts = backdating, the one thing the protocol's credibility forbids; a *separately-labeled* strategy-backtest surface is fine but must never merge into the on-chain line). Real fixes are forward (run refresher continuously — cluster 2 is it back on) + presentation (the windowing above).
- **Verification:** tsc 0 · vitest 88 · lint 0 · `next build` clean (29 routes). Committed on `live-pyth-spot` + redeployed to Vercel (https://noetrix.vercel.app).

### 2026-07-02 — Pitch-deck live-demo runbook (6-phase Workflow) + Pyth category went live on Sepolia
**Type:** Build (docs, multi-agent Workflow) under `/effort ultracode`. Capstone: the "real user demo workflow for the pitch-deck presentation." Also records a material state change — **the user broadcast `AddPythCategory` during co-editing, so the MNT_USD_SPOT Pyth category is now DEPLOYED on Mantle Sepolia**: `deployments/mantle-sepolia.json` gained `PythSpotResolver: 0x5CEa3D924c3f0Ac02BA4F17526c087D381bC5fF4` (registered on ResolutionEngine + PredictionMarket + CompositeFeed). MNT center tuned to ≈$0.50 (50_000_000 8-dec) across all agent configs + frontend mock. **Caveat: no *resolved* MNT_USD_SPOT track record yet** — agents forecast it on a synthetic seed; scores still accruing.
- **`docs/DEMO_WORKFLOW.md`** (new, 159 lines) — presenter runbook, 9 sections: TL;DR + money moment, real-user persona (Devi, treasury/risk at a Mantle yield protocol), pre-demo setup checklist, the demo arc as a timed table, expanded money moment, per-beat fallback plan (survives bots-down), competitive kill-shots (OnChain Radar/Mensa/OFT Sentinel/The Read), 8 honest judge-Q&A pairs, and an honesty ledger (live vs snapshot vs roadmap). `docs/DEMO_SCRIPT.md` now points to it (that stays the 2-min *recorded video* script).
- **How built:** `Workflow` tool, 6 phases / 13 agents (1.37M subagent tokens): Understand (4 parallel Explore readers grounding surfaces/proof/competitors/ops against the real repo) → Draft (3 arcs: data-buyer / anti-scam / benchmark-league) → Judge (panel scored each; **league won 44/50**) → Synthesize → **Verify (adversarial fact-check, 5 issues, verifyReady=false)** → Revise (applied all fixes). The verifier caught a real ship-blocker: the ranking cleanly reads naive<ARIMA<reasoner on the **AAVE** tab (reasoner #1), NOT METH — the runbook was corrected so "click the top agent" lands on `agent/2`.
- **Every load-bearing claim repo-verified post-run:** PythSpotResolver deployed (deployments JSON), IPFS trace CID `QmREF…3Wvy` real (`AgentDetailClient.tsx`), prediction #140 + resolutionBlock 39,649,841 in `insights-snapshot.json`, all addresses match deployments. (One Understand reader — ops_state — died without calling StructuredOutput → 3/4 grounded; the Verify pass reading deployments JSON backstopped it.)
- **Money moment:** agent/2 reasoning trace hash-committed to IPFS + anchored on-chain BEFORE the outcome (commit block < resolution block on Mantlescan `getPrediction(140)`) → independently verifiable, survives all bots being down.

### 2026-07-01 — Live Pyth spot category (keeper-snapshot resolver) + feed-chart honesty + simulator reframe + pre-commit review (branch `live-pyth-spot`)
**Type:** Build (contracts + frontend) + adversarial pre-commit review (4-lens Workflow). CEO-review strategy: "Sharpen Provable AI + 1 real resolver" to beat OnChain Radar/Mensa/OFT Sentinel/The Read on the Alpha & Data (Mirana) track. Spec: `docs/superpowers/specs/2026-07-01-live-pyth-spot-category-design.md` (updated this session to the shipped design). User drove A (Pyth contract layer) then B (chart honesty), plus flagged the simulator "stressed pays more" bug. Co-edited files live with the user (resolver + tests + PredictionMarket). Before committing, ran a 4-lens adversarial review workflow (contract-security / test-rigor / frontend / masterdoc) → 3 confirmed findings fixed (see "Pre-commit review" below).

**A — Pyth spot-price category, contract layer (DONE, 224 forge tests green):**
- **`interfaces/IPyth.sol`** — minimal ABI-compatible Pyth subset (`getPriceNoOlderThan`/`getUpdateFee`/`updatePriceFeeds`, `Price{int64 price;uint64 conf;int32 expo;uint256 publishTime}`). Real Pyth Mantle mainnet `0xA2aa501b19aff244D90cc15a4Cf739D2725B5729`; **Sepolia address still CONFIRM before deploy**. Feed IDs confirmed via Hermes 2026-07-01: MNT/USD `0x4e3037c8…fb0585`, ETH/USD fallback `0xff61491a…fd0ace`.
- **`mocks/MockPyth.sol`** — test double; absolute-diff staleness (future-dated prints past skew are stale too); `getUpdateFee` scales with update count (empty → 0).
- **`resolvers/PythSpotResolver.sol` — KEEPER-SNAPSHOT design** (`is ICategoryResolver, Ownable`). **The exploit a review caught & we closed:** the naive `view` live-read returns Pyth's latest at call time, but `ResolutionEngine.resolve` is **permissionless with no upper deadline** → an agent can wait for spot to drift into its own committed band and self-resolve into a near-max CRPS score. A "bounded live-read" only shrinks the window + adds a stuck-stake cliff. Fix: keeper `record(resolutionBlock, updateData)` pins the real Pyth print ONCE (onlyKeeper + first-write-wins) into `snapshotPrice[resolutionBlock]`; the `view` `resolve()` reads that fixed value → outcome is caller/timing-independent. `MAX_AGE=120s`, `MAX_CONF_BPS=500` (reject prints Pyth flags unreliable), `_to8dec` normalizes any expo, `setKeeper` owner-rotatable, excess fee refunded.
- **`PredictionMarket.voidExpired(predId)`** — liveness escape for the keeper-outage tail: controller-only, no slash/score, full refund once `block.number > resolutionBlock + VOID_DELAY_BLOCKS (43_200 ≈24h)`. Reuses Cancelled terminal state. (`cancel` can't cover it — blocked once `block.number >= resolutionBlock`.) **Resolvability-gated (added in pre-commit review):** before refunding, it `try`s `ICategoryResolver.resolve(value, resolutionBlock)` — if that SUCCEEDS the outcome exists, so it reverts `OutcomeAvailable()` and the prediction must be *resolved* (scored + slashed), not voided. Only a resolver revert (Pyth snapshot unrecorded / oracle down = a real outage) permits void. Closes the "heads I resolve, tails I void" escape — an agent can no longer void its losing forecasts to dodge the slash + reputation hit while self-resolving its winners. Also correctly strips the void path from the always-resolvable mETH/AAVE categories.
- **Tests:** `PythSpotResolver.t.sol` — 25 unit + 8 pipeline (33). Crown jewels: `test_Snapshot_ImmutableAgainstLaterPriceMoves`, `test_Record_FirstWriteWins`, `test_SnapshotPinned_TimingAndDriftIrrelevant` (grades pinned $0.80 near-max despite live drift to $4.00 — exploit closed at pipeline level), `test_VoidExpired_RevertsWhenResolvable` (recorded → void forbidden, resolves instead), + conf-gate/threshold/out-of-domain/normalization/void-escape. **Full suite 224 green** (was 207/209).

**B — composite-feed chart honesty (`FeedClient.tsx` + `feedView.ts`, frontend green):**
- **XAxis `type="number"` + linear numeric block domain** — real on-chain gaps now render as gaps, not the "cliff" (default *category* axis had spaced a ~540k-block gap as one even step).
- **`findLookbackPoint` tolerance** (default half the lookback) — "24h change" only shows when a genuine ≈24h-ago point exists; a nearest-point-across-a-gap now returns null → "—" (no fake delta). +2 vitest.
- **Contributor table reconciled to the on-chain KPI** — memo slices to exactly `latest.contributors` rows (user's edit); caption now keys off the ACTUAL rendered row count (`contributors.length`) and only claims "matching the Contributors count above" when they're equal (pre-commit-review fix — the old caption asserted a match unconditionally, false in the on-chain-fallback empty-table path + when qualifiers < KPI).

**Simulator "stressed pays more" fix + cheap reframe (`rwaSim.ts` + `DepositSimulator.tsx`):** root cause = displayed return was the nominal allocation-blend with NO confidence haircut, so stress just rotated to the higher-nominal-yield asset (USDY) and the number rose — backwards. Two-part fix: (1) added **`effectiveApyPct`** = allocation-blend haircut by per-asset confidence (mirrors on-chain `eff = yield × conf/1e4`), which falls monotonically under stress (+1 vitest, monotonic-decrease + ≤ nominal); (2) **reframed the outcome block (cheap, per the "concede the allocation lane to Mensa" directive)** to LEAD with the AI's protective *action* — an honest dollar allocation split ($X mETH / $Y USDY of the entered amount) + a stability-tradeoff caption ("trading mETH's upside for stability") — and demoted the rate to a supporting **risk-adjusted** line using `effectiveApyPct` (falls under stress), with the nominal blend shown only as its "before-discount" reference. The misleading big forward-$ hero (that rose under stress) is gone.

**Pre-commit review (4-lens adversarial Workflow → 3 confirmed, all fixed before commit):** (1) HIGH — `voidExpired` was a costless post-outcome escape (fixed via the resolvability gate above, +1 forge test); (2) MED — contributor caption falsely claimed "matching" over an empty/sliced table (fixed to key off `contributors.length`); (3) MED — masterdoc said "+3 vitest" for `findLookbackPoint` (actually +2; corrected). 1 finding dismissed on verify (the simulator `effectiveApyPct` masterdoc claim was accurate). Review surfaced by the `contract-security` + `frontend` + `masterdoc` lenses; each finding adversarially re-verified (refute-by-default) before acceptance.

**Verification:** forge **224** · frontend tsc 0 · lint 0 errors (12 warns all in unrelated generated file) · vitest **88** · `next build` clean (all routes incl `/terminal/feed/[category]`, `/terminal/simulation`).

**Deploy-ready wiring — BUILT this session (all typecheck/build green), one broadcast from live:**
- **SDK** `MNT_USD_SPOT` category (`categories.ts` domain `[0, 5e8]` 8-dec, 100 buckets) + barrel export.
- **`contracts/script/AddPythCategory.s.sol`** — standalone: deploys `PythSpotResolver(pyth, feedId, owner, keeper)`, registers MNT_USD_SPOT on ResolutionEngine + PredictionMarket + CompositeFeed (bounds), patches the `PythSpotResolver` key into `deployments/<net>.json` preserving the other 19. Env: `PYTH_ADDRESS` (CONFIRM Sepolia), `PYTH_FEED_ID` (MNT/USD default), `KEEPER` (defaults deployer). `forge build` clean.
- **Resolver bot keeper path** (`agents/resolver/{config,index}.ts`) — optional `PythConfig` (undefined ⇒ plain resolve path unchanged); for a matured MNT_USD_SPOT prediction it fetches a Hermes update, `getUpdateFee`, `record{value:fee}(rb, updateData)` (skips if `recorded(rb)`), then resolves. `record` failure ⇒ skip + retry (void escape covers permanent). `.env.example` documents `PYTH_ADDRESS`/`PYTH_FEED_ID`/`HERMES_URL`; bot wallet must be the resolver's keeper.
- **Agent configs** — MNT_USD_SPOT added to arima/naive/swarm-runner (`CategorySeed` center 80_000_000 ≈ $0.80 8-dec) + deepseek-reasoner (`CategoryDef` w/ Pyth-verifiable description). Agents forecast it on synthetic seed until real history accrues.
- **Frontend tab** — `CategoryId` + `CATEGORIES` MNT_USD_SPOT (unit "usd", price `unitFormatter` `$X.XXXX`); all `Record<CategoryId,…>` maps filled (labels/DOMAIN/gen-insights/8 mock-agent reputations via a shared `MNT_COLD` calibrating cold-start; `makeFeedHistory` price-scale branch).
- **Verification:** forge 223 · SDK+resolver+4 agents tsc 0 · frontend tsc 0 · vitest 88 · lint 0 · next build ✓ (29 pages).

**Pending (operational, needs keys/host):** confirm the Mantle Sepolia Pyth address → `forge script AddPythCategory --broadcast` → set the bot wallet as keeper → run agents + resolver (keeper mode) + refresher cadence → grow ≥10 resolved → regen snapshots + Vercel. **Deferred (quality):** `market-data` MNT/USD real price fetcher (agents on synthetic seed for now); backtest disagreeScale calibration for the feed. **Next build:** positioning reframe (benchmark/Turing-Test) + demote RWA-allocation surfaces; legible verifiable-backtest surface; "AI Reasoner" archetype rename. **Capstone still queued:** real user demo workflow for the pitch-deck presentation.
...
**Audit findings (verdicts: lifecycle SOUND-with-gaps · bots BROKEN-cold-start/FRAGILE · CI none · frontend/deploy SOUND-with-gaps):**
- **Feed liveness is the central fragility (HIGH):** (1) deployed CompositeFeed lacks hold-last-good (confirmed again — feed now ~52h stale, nonzero only by luck); (2) **STRUCTURAL flaw the patch does NOT fix** — `CompositeFeed.sol:176` counts only still-`Revealed` predictions, so even with hold-last-good the feed needs a continuously in-flight forecast or it goes stale and RiskManager Freezes / YieldAllocator→50/50; (3) **SPOF** — `resolve()`/`refresh()` are permissionless-but-incentiveless and in prod called only by two single laptop bots (stopped). Real fix = host bots + (ideally) aggregate latest-regardless-of-resolve-status.
- **Bot orchestration BROKEN for cold-start:** deepseek-reasoner has no `.env` in its own dir (live key stranded in stale `claude-reasoner/`); naive (id3) has no runnable `.env`; forecaster modes desynced on disk (arima=normal/24h vs reasoner=seed/12min → short demo shows 2 of 3 agents); resolver cursor wedges on any stuck non-terminal prediction + re-scans 171 reads/tick (no multicall); no supervisor/hosting.
- **Zero CI/automation (HIGH for drift):** no `.github/`, no husky — exactly what let doc-counts drift; on-chain↔frontend address coupling is hand-copy + fails silently; **2 leftover worktrees** (`.claude/worktrees/agent-swarm-onchain` is **12 commits unmerged**; `.worktrees/terminal-entry-nav` 2 commits); master unpushed.
- **Economic loop dead (MED):** finalizeEpoch/claimBonus never automated → slashed MNT locked in BonusDistributor.
- **Deploy runway VALIDATED correct where it counts:** adversarial review confirmed `RedeployFeed.s.sol` JSON round-trip preserves all 18 keys (no corruption), all `verify.sh` constructor-arg encodings correct, consumer re-wiring + RiskManager params match Deploy.s.sol, gate-open doesn't brick reads. Found only doc bugs in the runbook (mine).

**Fixes applied (6, all verified green):**
- **`/insights` snapshot honesty** (`hooks.ts` + `InsightsClient.tsx`): `useInsightsData` now returns `source:"snapshot"` (was hardcoded `"live"` over a frozen build-time snapshot); pill relabeled "Live on-chain data"→"On-chain snapshot" (no pulse) + a "captured Nd ago" age badge (warn-tone if >24h), computed in a deferred-setState effect (React-Compiler-safe). `"snapshot"` was already a first-class `DataSource` (LeaderboardClient uses it) — no type ripple.
- **`/pricing` subscribe guard** (`PricingClient.tsx`): Subscribe button now `disabled` when `!hasSubscriptionGate` (previously a connected user with gate unset could click → writeContract to empty addr → revert).
- **Resolver redeploy clamp** (`agents/resolver/src/index.ts`): auto-resets cursor to 1 when `cursor > nextPredictionId` (a redeployed PredictionMarket) — removes the manual "delete resolver.state.json on redeploy" wedge.
- **Runbook corrections** (`docs/DEPLOY.md`): "13"→"13 contract addresses + chainId"; indexer is NOT "just a restart" (needs fresh `--schema liveN` + clear `.ponder/` on address change); Step 5 (snapshot regen) MUST precede Step 6 build (`/api/leaderboard` statically imports the JSON); verify.sh marked authoritative.
- **RedeployFeed env-file output** (`contracts/script/RedeployFeed.s.sol`): now writes ready-to-paste env lines to `deployments/<net>-redeploy-env.txt` (within the fs-write sandbox) instead of only logging; "13"→"14" comment fix. `forge build` clean.

**Verification:** forge build (RedeployFeed) clean · resolver tsc 0 · frontend tsc 0 · lint 0 · vitest 55 · e2e **9/9**.

**Deferred (bigger, needs decision/keys — NOT done):** add CI (one `.github/workflows/ci.yml`: forge test + frontend gates); host the 5 bots + indexer off-laptop; the structural feed-liveness redesign (aggregate latest-regardless-of-resolve); automate finalizeEpoch; resolve/delete the 2 leftover worktrees; push master. These are the recommendations from the workflow audit, captured for a future session.
...
- **(C) AI-agent interface layer** (user ask — "can agents interact with the site?"; assume RealClaw/openClaw + any agent). Tested: protocol already agent-native on-chain (SDK + ERC-8004), but the *website* exposed nothing over HTTP. Added: `public/llms.txt` + `public/robots.txt`; `public/.well-known/agents.json` (machine manifest — contract addresses, category ids w/ keccak hashes, 5-step participate, endpoints; Next serves the dotfolder fine); `/api/leaderboard` (serves committed snapshot, CORS `*`) + `/api/feed` (**live on-chain `CompositeFeed.read` via viem**, CORS `*`); `/agents` human page (machine + on-chain interface, nav→More). All live-tested: leaderboard returns ARIMA/DeepSeek, feed returned 3081 bps / 2 contributors. NOTE: frontend tsconfig target <ES2020 → use `BigInt(0)` not `0n` in routes. Build 17 routes.
