...
- **Deferred to Batch B (operational, needs server/keeper — surfaced to user):** `forge verify-contract` PythSpotResolver; actually RUN MNT_USD_SPOT (agent forecasts → keeper `record()` → resolve ≥1 → refresh) so it's genuinely live; regen the 21-day-old insights snapshot; set OPENROUTER_API_KEY in Vercel if real AI compliance is wanted.
