...
import { loadConfig, type ArimaConfig, type CategorySeed } from "./config.js";
import { loadState, saveState, type AgentState } from "./state.js";
import { arima111 } from "./arima.js";
...
function checkAutoFlip(state: AgentState, resolvedCount: number, now: number): AgentState {
  ...
  if (resolvedCount >= FLIP_RESOLVED_THRESHOLD || elapsed > FLIP_ELAPSED_SECONDS) {
    console.log(
      `[arima] flipping seed → normal (resolved=${resolvedCount}, elapsed=${Math.round(elapsed / 3600)}h)`,
    );
...
async function submitForCategory(
  ...
  if (series.length < 10) {
    console.log(`[arima] ${seed.label}: only ${series.length} historical points — using synthetic seed data`);
    series = syntheticSeries(seed);
    usedSynthetic = true;
  }

  const forecast = arima111(series, 1);
  ...
  const content = {
    ...
    forecast: {
      ...
      params: { phi: forecast.phi, theta: forecast.theta, c: forecast.c },
    },
  ...
  console.log(
    `[arima] ${seed.label}: forecast=${Math.round(forecast.mean)} band=[${range.low},${range.high}] ` +
  ...
  console.log(
    `[arima] ${seed.label}: submitted prediction ${result.predictionId} (commit ${result.commitTx.slice(0, 10)}, reveal ${result.revealTx.slice(0, 10)})`,
  );
}

async function tick(agent: Agent, cfg: ArimaConfig): Promise<number> {
  let state = loadState();
  ...
  console.log(`[arima] tick — mode=${state.mode} offset=${offset} resolved=${resolvedCount}`);

  for (const seed of cfg.categories) {
    try {
      await submitForCategory(agent, cfg, seed, offset);
    } catch (err) {
      console.error(`[arima] ${seed.label} submission failed:`, (err as Error).message);
    }
  }
...
async function main(): Promise<void> {
  ...
  console.log(`[arima] starting — agentId=${cfg.agentId} controller=${agent.account.address}`);
  ...
  while (true) {
    ...
    try {
      ...
      console.error(`[arima] tick failed:`, (err as Error).message);
    }
    console.log(`[arima] next tick in ${Math.round(nextDelay / 60000)} min`);
